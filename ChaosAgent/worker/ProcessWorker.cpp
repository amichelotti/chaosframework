...
void ProcessWorker::timeout() {
    ...
    for(MapRespawnableNodeIterator it = map_respawnable_node().begin(),
        ...
        if(CHECK_PROCESS(it->second) == false) {
            INFO << CHAOS_FORMAT("Respawn process for node %1%", %it->first);
            LAUNCH_PROCESS(it->second);
        }
...
CDWUniquePtr ProcessWorker::stopNode(CDWUniquePtr data) {
    AgentAssociationSDWrapper assoc_sd_wrapper;
    assoc_sd_wrapper.deserialize(data.get());
    if(CHECK_PROCESS(assoc_sd_wrapper()) == true) {
        bool kill = false;
       ...
       QUIT_PROCESS(assoc_sd_wrapper(), kill);
    } else {
...
CDWUniquePtr ProcessWorker::restartNode(CDWUniquePtr data) {
    ...
    removeToRespawn(assoc_sd_wrapper().associated_node_uid);
    if(CHECK_PROCESS(assoc_sd_wrapper()) == true) {
       QUIT_PROCESS(assoc_sd_wrapper());
        while((process_alive = CHECK_PROCESS(assoc_sd_wrapper())) == false &&
              try_to_stop) {
            if(retry++ > 3) {
                try_to_stop = false;
                //try to kill process
               QUIT_PROCESS(assoc_sd_wrapper(), true);
                //exit without check
            } else {
                sleep(1);
            }
    ...
    if((process_alive = process_alive || CHECK_PROCESS(assoc_sd_wrapper())) == false) {
