  ...
  try {
    ...
    if (needAnswer && remote_action_result.get()) {
      ...
      do {
        ...
        if (retry == 1) {
          LDBG_ << answer_id << "] Sending back error packet:" << response_pack->getJSONString();
        }
        if ((!(ret = submitMessage(answer_ip,
                                   MOVE(response_pack),
                                   false)))) {
          LERR_ << answer_id << "] Error submitting answer to '" << answer_ip << "' retry: " << retry;
        }
...
CDWUniquePtr SharedCommandDispatcher::dispatchCommand(CDWUniquePtr rpc_call_data) {
  ...
  try {
    ...
    if (map_domain_actions().count(actionDomain) == 0) throw CException(ErrorRpcCode::EC_RPC_NO_DOMAIN_REGISTERED_ON_SERVER, "Action Domain \"" + actionDomain + "\" not registered (cmd pack \"" + rpc_call_data->getJSONString() + "\")", __PRETTY_FUNCTION__);
