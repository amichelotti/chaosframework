...
#define EVT_DATA_MAX_BIT_LENGTH         8152 //1021 byte length for data
    ...
    public:
        ...
        /*
         ...
         \param existingEventData pointer to an event data, the buff this is length 1024 byte
         */
        EventDescriptor();
        ...
        static EventDescriptor *initDataWithMemory(const unsigned char * const existingEventData)  {
            ...
            EventDescriptor *newInstance = new E(existingEventData == NULL);
            if(existingEventData != NULL){
                //copy the initialization buffer
                memcpy(newInstance->eventData, existingEventData, EVENT_DATA_BYTE_LENGTH);
            }
        }
