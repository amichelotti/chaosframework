...
HealtManagerBase::HealtManagerBase()
    : mds_message_channel(NULL), last_fire_counter_set(0), current_fire_slot(0) {}
...
void HealtManagerBase::addNewNode(const std::string& node_uid) {
  ...
  {
   ChaosLockGuard wl(map_node_mutex);
  ...
  HM_DBG << "Added new health dataset for node :'" << node_uid << "' with push counter of " << healt_metric->fire_slot;
}
...
void HealtManagerBase::addNodeMetric(const std::string&        node_uid,
  ...
  ChaosLockGuard rl_map(map_node_mutex);
...
void HealtManagerBase::addNodeMetricValue(const std::string& node_uid,
                                          ...
                                          bool               publish) {
  //  try{
  ChaosSharedPtr<NodeHealtSet> node_metrics_ptrp;
  ...
  // boost::shared_lock<boost::shared_mutex> wl(map_node_mutex);
  {
    ...
    // read lock
    node_metrics_ptrp = map_node[node_uid];
  }
  //
  // wl.unlock();

  // work on local shared pointer
  HEALT_SET_METRIC_TIMESTAMP_LAST_METRIC(node_metrics_ptrp);
  HEALT_SET_METRIC_VALUE(node_metrics_ptrp,
                         Int32HealtMetric,
                         node_metric,
                         int32_value);
  if (publish) {
    publishNodeHealt(node_uid);
  }
  // }CHAOS_BOOST_LOCK_EXCEPTION_CACTH(lock_exception,)
}
void HealtManagerBase::addNodeMetricValue(const std::string& node_uid,
  ...
  // boost::shared_lock<boost::shared_mutex> wl(map_node_mutex);
  ChaosSharedPtr<NodeHealtSet> node_metrics_ptr;
  {
    ChaosLockGuard wl(map_node_mutex);
    HEALT_NEED_NODE_AND_METRIC_PRESENCE(node_uid, node_metric)
    // read lock
    node_metrics_ptr = map_node[node_uid];
  }
  // unlock all map
  //  wl.unlock();
  // work on local shared pointer
...
void HealtManagerBase::publishNodeHealt(const std::string& node_uid) {
  ChaosLockGuard rl(map_node_mutex);
...
void HealtManagerBase::_publish(const ChaosSharedPtr<NodeHealtSet>& heath_set,
  ...
  // ChaosLockGuard wl_io(mutex_publishing); // may lock all
